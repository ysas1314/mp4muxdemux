...
int MP4Encoder::WriteH264Data(MP4FileHandle hMp4File, const unsigned char *pData, int size) {
    ...
    while (len = ReadOneNaluFromBuf(pData, size, pos, nalu)) {
        pos += len;
        if (nalu.type == 0x07 && m_videoId == MP4_INVALID_TRACK_ID) {// sps
            // 添加h264 track
            m_videoId = MP4AddH264VideoTrack(hMp4File, m_nTimeScale, m_nTimeScale / m_nFrameRate ,m_nWidth, m_nHeight,nalu.data[1], nalu.data[2], nalu.data[3], 3);           // 4 bytes length before each NAL unit
            if (m_videoId == MP4_INVALID_TRACK_ID) {
                printf("add video track failed.\n");
                return 0;
            }
            ...
            if ( nalu.type  == 0x06 || nalu.type  == 0x07 || nalu.type == 0x08 || nalu.type == 0x09) {
                continue;
            }
...
bool MP4Encoder::WriteH264File(const char *pFile264, const char *pFileMp4) {
    ...
    MP4FileHandle hMp4File = CreateMP4File(pFileMp4, 1280, 720, 30000, 15);
